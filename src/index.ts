...
function rmParentDir(path: string) {
    const parentDir = nodePath.dirname(path);
    try {
        fs.rmdirSync(parentDir);
        rmParentDir(parentDir);
    } catch {
        /* Do nothing */
    }
}
...
export class PasswordStore {
    ...
    public constructor(private privateKeyFile: string, passwordStoreDir?: string) {
        this.kbpgp = require("kbpgp");
        this.passwordStoreDir = passwordStoreDir ?? nodePath.join(os.homedir(), ".password-store");
                    ...
                    key.sign({}, (err: Error | null) => {
                        ...
                        key.export_pgp_private({}, (err: Error | null, pgpPrivate: string) => {
                            ...
                            key.export_pgp_public({}, (err: Error | null, pgpPublic: string) => {
                                if (err) return null;
                                fs.writeFileSync(this.privateKeyFile + ".pub", pgpPublic, {
                                    mode: 0o644,
                                });
                                this.deferred.resolve();
                            });
    ...
    public static async create(privateKeyFile: string, passwordStoreDir?: string): Promise<PasswordStore> {
        ...
        await instance.deferred.promise;
        return instance;
    }

    public async getPassword(service: string, account: string): Promise<string | null> {
        await this.deferred.promise;
        const passwordFile = nodePath.join(this.passwordStoreDir, service, account) + ".gpg";
    ...
    public async setPassword(service: string, account: string, password: string): Promise<void> {
        await this.deferred.promise;
        const passwordFile = nodePath.join(this.passwordStoreDir, service, account) + ".gpg";
        fs.mkdirSync(nodePath.dirname(passwordFile), {
            mode: 0o700,
            recursive: true,
        });
        return new Promise((resolve, reject) => {
    ...
    public async findCredentials(service: string): Promise<{ account: string; password: string }[]> {
        await this.deferred.promise;
        const serviceDir = nodePath.join(this.passwordStoreDir, service);
